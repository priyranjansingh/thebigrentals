...
class Coupons extends BaseModel
{
	...
	public function rules()
	{
		...
		return array(
			array('id, coupon_code, type, value, valid_till, created_by, modified_by, created_date, modified_date', 'required'),
			array('status, deleted', 'numerical', 'integerOnly'=>true),
			array('id, created_by, modified_by', 'length', 'max'=>36),
			array('coupon_code, value', 'length', 'max'=>16),
			array('type', 'length', 'max'=>1),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, coupon_code, type, value, valid_till, created_by, modified_by, created_date, modified_date, status, deleted', 'safe', 'on'=>'search'),
		);
	}
