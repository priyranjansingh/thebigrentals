...
class DefaultController extends Controller {
        ...
        if (!isFrontUserLoggedIn()) {
            ...
            if (isset($_POST['Registration'])) {
                ...
                if ($model->validate()) {
                    ...
                    $url = $_SERVER['SERVER_NAME'] . $url;
                    $to = $model->email;
                    $subject = 'The Big Rentals Account Activation';
                    $message = getVerifyEmail($url);
                    mailsend($to, "arommatech@gmail.com", $subject, $message);
                    $this->afterRegister = true;
                    $this->redirect(array('success', "for" => 'registeration'));
                    // end of mail sending
    ...
    public function actionSuccess($for) {
        if ($for == "registeration")
...
//                    $user_image = uploadFile($image_name, $image_type, $image_tmp_name, 'users');
    ...
    public function actionCropImage() {
        ...
        $src = './assets/images/' . $_POST['hid_name'];
        $img_r = imagecreatefromjpeg($src);
        $dst_r = ImageCreateTrueColor($targ_w, $targ_h);

        imagecopyresampled($dst_r, $img_r, 0, 0, $_POST['x'], $_POST['y'], $targ_w, $targ_h, $_POST['w'], $_POST['h']);
        $destination_url = "./assets/thumb/" . $_POST['hid_name'];
        imagejpeg($dst_r, $destination_url, $jpeg_quality);
        $user_image = uploadToS3Thumb("./assets/thumb/" . $_POST['hid_name'], "thumb_" . $_POST['hid_name']);
        $id = frontUserId();
        $model = Profile::model()->findByPk($id);
        $model->user_image = $user_image;
        if ($model->validate()) {
        ...
        $this->redirect(base_url() . '/user/myaccount');
    }
    ...
    public function actionChangepassword() {
        ...
        if (isFrontUserLoggedIn()) {
            ...
            if (isset($_POST['ChangeUserPassword'])) {
                ...
                if ($model->validate()) {
//                    $new_password = Profile::model()->findbyPk($id);
    ...
    public function actionajaxfileupload() {
        if (!empty($_FILES['myfile']['tmp_name'])) {
                ...
                if ($width > $thumbWidth) {
                    ...
                    if ($extension == 'JPG' || $extension == 'JPEG') {
                        imagejpeg($tmp_img, "./assets/images/" . $_FILES['myfile']['name']);
                    } else if ($extension == 'GIF') {
                        imagegif($tmp_img, "./assets/images/" . $_FILES['myfile']['name']);
                    } else if ($extension == 'PNG') {
                        imagepng($tmp_img, "./assets/images/" . $_FILES['myfile']['name']);
                    }
                } else {
                    copy($_FILES['myfile']['tmp_name'], "./assets/images/" . $_FILES['myfile']['name']);
                }
